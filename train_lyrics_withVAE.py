...
def train_loop(data, model, optimizers, schedulers, loss_functions, loss_weights):
    ...
    features, pitches, onsets, input_lengths, tatum_frames, texts, text_lengths = data
    input_features, target_features = features
    input_features = input_features.to(DEVICE)
    target_features = target_features.to(DEVICE)
    
    # Forward lyrics
    lyrics_logits = model['lyrics'](target_features)
    # lyrics_logits: (num_frames, batch_size, num_classes)
    ...
    loss_reconst = loss_functions['reconst'](
            ...
            tatum_frames,
            supervised=False,
            lyrics=lyrics_logprobs.transpose(0, 1),
            lyrics_downsample=model['lyrics'].down_sample,
        )
    
    if loss_weights['reconst'] != 0.:
        loss = loss_weights['lyrics'] * loss_lyrics + loss_weights['reconst'] * loss_reconst    
    else:
        loss = loss_weights['lyrics'] * loss_lyrics
...
@test_loop_decorator
def test_loop(data, model, dataloader_name, loss_functions, decoder, codec):
    features, pitches, onsets, input_lengths, tatum_frames, texts, text_lengths = data
    input_features, target_features = features
    input_features = input_features.to(DEVICE)
    target_features = target_features.to(DEVICE)
