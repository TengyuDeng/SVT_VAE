...
def get_CTC_decoder(blank_id, **configs):
    decoder_type = configs.pop("type")
    if decoder_type == "greedy":
        return GreedyCTCDecoder(blank_id, **configs)
    elif decoder_type == "ds_beam":
        return DSDecoder(blank_id, **configs)
    elif decoder_type == "ctc_decode":
        return CTCDecoder(blank_id, **configs)
    else:
...
class DSDecoder:
    ...
    def decode_one(self, softmax_out):
        
        softmax_out = np.append(softmax_out[:, 1:], softmax_out[:, (0,)], axis=-1)
        # softmax_out: (length, num_class)
        
        predict = ctc_beam_search_decoder(probs_seq=softmax_out,
                                          alphabet=self.alphabet,
                                          scorer=self.scorer,
                                          beam_size=25,
                                         )[0][1]
...
class CTCDecoder:
    ...
    def __init__(self, blank_id, target_type="word", model_path=None, beam_width=25, **args):
        if target_type == "phoneme":
            self.g2p = G2p()
            self.characs =['|', ' '] + self.g2p.phonemes[4:]
        else:
            self.characs = self.characs = "| abcdefghijklmnopqrstuvwxyz'"
    ...
    # def decode(self, probs, input_lengths):
    def decode(self, probs):
        predicts = []
        beam_results, beam_scores, timesteps, out_lens = self.decoder.decode(probs)
        for i in range(probs.shape[0]):
            predict = beam_results[i, 0, :out_lens[i, 0]]
            if len(predict) == 0:
                predict = torch.tensor([1])
            predicts.append(predict)
        # for i in range(probs.shape[0]):
        #     beam_results, beam_scores, timesteps, out_lens = self.decoder.decode(probs[(i,), :input_lengths[i], :])
        #     predict = beam_results[0, :out_lens[0]]
        #     if len(predict) == 0:
        #         predict = torch.tensor([1])
        #     predicts.append(predict)
    ...
    def decode_one(self, probs):
        if probs.ndim == 2:
            probs = probs.unsqueeze(0)
        beam_results, beam_scores, timesteps, out_lens = self.decoder.decode(probs)
        predict = beam_results[0, 0, :out_lens[0, 0]]
        if len(predict) == 0:
            predict = torch.tensor([1])
